using System.Text.RegularExpressions;
using Wordki.Bff.SharedKernel.Results;

namespace Wordki.Modules.Users.Application.Commands.RegisterUser;

public static partial class RegisterUserCommandValidator
{
    public static Result Validate(RegisterUserCommand command)
    {
        var errors = new List<AppError>();

        if (string.IsNullOrWhiteSpace(command.Email))
        {
            errors.Add(new AppError(
                "users.validation.email.required",
                "Email is required.",
                ErrorType.Validation,
                "email"));
        }

        if (!string.IsNullOrWhiteSpace(command.Email) && !EmailRegex().IsMatch(command.Email.Trim()))
        {
            errors.Add(new AppError(
                "users.validation.email.invalid",
                "Email format is invalid.",
                ErrorType.Validation,
                "email"));
        }

        if (string.IsNullOrWhiteSpace(command.Password))
        {
            errors.Add(new AppError(
                "users.validation.password.required",
                "Password is required.",
                ErrorType.Validation,
                "password"));
        }

        if (!string.IsNullOrWhiteSpace(command.Password) && command.Password.Length < 8)
        {
            errors.Add(new AppError(
                "users.validation.password.too_short",
                "Password must have at least 8 characters.",
                ErrorType.Validation,
                "password"));
        }

        if (string.IsNullOrWhiteSpace(command.UserName))
        {
            errors.Add(new AppError(
                "users.validation.username.required",
                "User name is required.",
                ErrorType.Validation,
                "userName"));
        }

        if (!string.IsNullOrWhiteSpace(command.UserName) && command.UserName.Trim().Length > 100)
        {
            errors.Add(new AppError(
                "users.validation.username.too_long",
                "User name cannot exceed 100 characters.",
                ErrorType.Validation,
                "userName"));
        }

        return errors.Count == 0
            ? Result.Success()
            : Result.Failure(errors);
    }

    [GeneratedRegex(@"^[^@\s]+@[^@\s]+\.[^@\s]+$", RegexOptions.Compiled | RegexOptions.IgnoreCase)]
    private static partial Regex EmailRegex();
}
